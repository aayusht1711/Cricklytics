...
def show_scouting_view(data):
    ...
    if generate:
        ...
        <head>
            <style>
                body {{ font-family: 'Arial', sans-serif; background-color: #f4f4f9; color: #333; padding: 40px; line-height: 1.6; }}
                h1 {{ color: {t_color}; border-bottom: 2px solid {t_color}; padding-bottom: 10px; }}
                .stat-box {{ background: #fff; border-radius: 8px; padding: 20px; margin-bottom: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }}
                .stat-grid {{ display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; text-align: center; }}
                .stat-item h3 {{ margin: 0; font-size: 14px; color: #666; }}
                .stat-item p {{ margin: 5px 0 0; font-size: 24px; font-weight: bold; color: #222; }}
                .report-text {{ background: #fff; padding: 25px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); white-space: pre-wrap; }}
                .footer {{ margin-top: 40px; font-size: 12px; color: #888; text-align: center; }}
            </style>
        </head>
        <body>
            <h1>{player.upper()} - Official Scouting Report</h1>
            <p><strong>Franchise:</strong> {profile['team']} | <strong>Seasons:</strong> {profile['seasons']}</p>
            
            <div class="stat-box">
                <div class="stat-grid">
                    <div class="stat-item"><h3>Runs</h3><p>{profile['runs']:,}</p></div>
                    <div class="stat-item"><h3>Strike Rate</h3><p>{profile['sr']}</p></div>
                    <div class="stat-item"><h3>Average</h3><p>{profile['avg']}</p></div>
                    <div class="stat-item"><h3>Sixes</h3><p>{profile['sixes']}</p></div>
                </div>
            </div>
            
            <div class="stat-box">
                <div class="stat-grid">
                    <div class="stat-item"><h3>Powerplay SR</h3><p>{profile['pp_sr']}</p></div>
                    <div class="stat-item"><h3>Middle SR</h3><p>{profile['mid_sr']}</p></div>
                    <div class="stat-item"><h3>Death SR</h3><p>{profile['death_sr']}</p></div>
                    <div class="stat-item"><h3>Knockout SR</h3><p>{profile['ko_sr']}</p></div>
                </div>
            </div>
            
            <div class="report-text">
                <h2 style="color: #333; margin-top: 0;">AI Analysis & Verdict</h2>
                {report}
            </div>
            
            <div class="footer">Generated by Cricklytics Engine | Aayush Tripathi</div>
        </body>
        </html>
        """
        
        b64_html = base64.b64encode(html_content.encode("utf-8")).decode()
        dl_link = f'<a href="data:text/html;base64,{b64_html}" download="{player.replace(" ", "_")}_Scouting_Report.html" style="display:inline-block; margin-top:10px; margin-bottom:20px; padding:10px 20px; background-color:{t_color}; color:#fff; text-decoration:none; border-radius:5px; font-weight:bold;">📥 Download HTML Report</a>'
